...
ファイル名は必ず以下を守ること。新規作成・リネームのときだけでなく、スクリプトやツールが生成する名前を扱うときも同様。

- **全角記号を使わない**：`（）`・`／`・`・`・`：`・`　`（全角スペース）など。記号は **半角のみ** を使う
- **半角であっても、シェルやパスでエスケープが必要になる記号は使わない**：ファイル名は検索・参照する機会が多いため
  - 禁止例：`/` `\` スペース `"` `'` `` ` `` `*` `?` `[` `]` `(` `)` `{` `}` `<` `>` `|` `&` `;` `$` `!` `#`
- **区切りには `-`（ハイフン）か `_`（アンダースコア）を使う**
- 迷ったら「半角英数字 ＋ `-` ＋ `_`」だけで構成する。日本語（かな・漢字）自体は可

例：
- NG：`面談メモ（AI活用プロダクト開発案件）.md` ／ `出張メモ（8/27-29 五反田）.md`
- OK：`面談メモ-AI活用プロダクト開発案件.md` ／ `出張メモ-8-27-29-五反田.md`
...
- `title` は内容が一目でわかる短いものにする。日本語推奨だが、検索性を優先したい場合は半角英数（ハイフン区切り）も可（例：`"api認証エンドポイント追加の方針"` / `"api-auth-endpoint-plan"`）
- 本文には少なくとも以下を含める：
  - 背景・目的
  - 採用方針と却下した代替案、その理由
...
コミットを伴う変更作業を行う際は、**必ず `main` から作業ブランチを切ってから commit すること**。`main` への直 commit / 直 push は禁止（読み取り作業や `git status` などの非変更操作は対象外）。
...
| `revert` | 過去のコミット／PR を巻き戻す変更（GitHub の "Revert" ボタンが生成する `revert-<num>-<branch>` 形式もこれに準ずる） |

- 説明部分は **英小文字・数字・ハイフン**（`[a-z0-9-]+`）のみ。長さは prefix を含めて **〜50 字程度** を目安にする
- 例：`feat/add-auth-endpoint`、`fix/serial-number-parse-error`、`fix/issue-42-token-refresh`
...
- **`main` に直 commit / 直 push しない**：必ず作業ブランチを切って PR 経由でマージする（詳細は「ブランチ戦略」セクション参照）
...
ユーザーが「〜しておくね」「〜するね」「あとで〜する」のように **主語がユーザー自身** の宣言をしたときは、代わりに実行しないこと。実行するかどうか・いつ実行するかはユーザーが決めること。

同様に、エージェント側の操作ミスを指摘されたときは、**リカバリ（取り消し・修正・再投稿）も勝手に判断しない**。「取り消しますか？」「このまま残しますか？」と確認してから動くこと。ミスのリカバリ判断もユーザーの裁量。

NG 例：
- ユーザー「approve しておくね」→ エージェントが `gh pr review --approve` を実行する
- ユーザー「勝手にやるな」→ エージェントが即座に dismiss する

OK 例：
- ユーザー「approve しておくね」→ 何もせず次の指示を待つ
- 誤操作を指摘された → 「取り消しますか？それとも残しますか？」と確認してから動く
